...
class GasState(FlowState):
    ...
    def from_TP(cls, T, P, m_dot=None, composition=None, name="", ref=None):
        ...
        if _is_missing(T) or _is_missing(P) or composition is None:
            return cls(name=name, T=T, P=P, m_dot=m_dot, h=None, s=None, ref=ref, composition=composition or GasComposition())
        try:
            h, s, cp = mixture_h_s_cp(T, P, composition)
        except Exception:
            return cls(name=name, T=T, P=P, m_dot=m_dot, h=None, s=None, ref=ref, composition=composition)
        return cls(
            name=name,
            T=T,
            P=P,
    ...
    def from_Ph(cls, P, h, m_dot=None, composition=None, name="", ref=None):
        if _is_missing(P) or _is_missing(h) or composition is None:
            return cls(name=name, P=P, m_dot=m_dot, h=h, s=None, ref=ref)
        composition = composition.normalized()
        try:
            T = solve_temperature_from_property(P, h, composition, "h")
        except Exception:
            return cls(name=name, P=P, m_dot=m_dot, h=h, s=None, ref=ref, composition=composition)
    ...
    def from_Ps(cls, P, s, m_dot=None, composition=None, name="", ref=None):
        if _is_missing(P) or _is_missing(s) or composition is None:
            return cls(name=name, P=P, m_dot=m_dot, h=None, s=s, ref=ref)
        composition = composition.normalized()
        try:
            T = solve_temperature_from_property(P, s, composition, "s")
        except Exception:
            return cls(name=name, P=P, m_dot=m_dot, h=None, s=s, ref=ref, composition=composition)
        return cls.from_TP(T, P, m_dot=m_dot, composition=composition, name=name, ref=ref)
