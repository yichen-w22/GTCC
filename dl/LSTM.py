...
feature_cols = list(data.columns[:-5])   # 你也可以改成自己选的列名 list
...
# =========================
# 1) 生成 LSTM 序列数据：X_seq -> y
#    用过去 seq_len 个时刻预测当前时刻输出
# =========================
seq_len = 30  # 你可以先用 30（表示用过去30分钟预测当前）
...
# X_seq: (N, seq_len, n_features), y_seq: (N, n_targets)

print("X_seq shape:", X_seq.shape, "y_seq shape:", y_seq.shape)


# =========================
# 2) 划分 train / val / test（不打乱，按时间切）
# =========================
...
class LSTMNet(nn.Module):
    ...
    def forward(self, x):
        ...
        last = out[:, -1, :]           # 取最后一个时间步
        yhat = self.fc(last)           # (batch, out_dim)
        return yhat
...
# 反归一化（注意：y_test/y_pred_test 是 (N, out_dim) 仍然能 inverse_transform）
