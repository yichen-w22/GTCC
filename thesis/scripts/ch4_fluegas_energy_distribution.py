"""Chapter 4 — 余热锅炉烟气侧能量分配 (1烟气能量分配.png)"""
...
def main() -> None:
    ...
    layers = [
        (data["generation_mw"], "汽机发电"),
        (data["generation_mw"] + data["stack_loss_mw"], "余热锅炉出口尾气损失"),
        (data["generation_mw"] + data["stack_loss_mw"] + data["condenser_mw"], "热网供热 + 凝汽器放热"),
    ]
    ...
    fig.savefig(OUTPUT_DIR / "1烟气能量分配.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close(fig)
    print(f"已保存 {OUTPUT_DIR / '1烟气能量分配.png'}")
