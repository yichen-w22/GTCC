"""Chapter 4 — 汽轮机各缸出力分配 (4各缸出力分配.png)"""
...
def main() -> None:
    ...
    for y, label in [(hp_top, "高压缸出力"), (ip_top, "中压缸出力"), (lp_top, "低压缸出力")]:
    ...
    fig.savefig(OUTPUT_DIR / "4各缸出力分配.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close(fig)
    print(f"已保存 {OUTPUT_DIR / '4各缸出力分配.png'}")
